...
import { Response as SurveyResponse, ChatMessage } from "@/types/database";
...
export default function HistoricoPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setResponses((resps ?? []) as SurveyResponse[]);
      ...
      const grouped = (msgs ?? []).reduce<Record<string, { count: number; last: string }>>((acc, m) => {
        if (!acc[m.session_id]) acc[m.session_id] = { count: 0, last: m.created_at };
        acc[m.session_id].count++;
        return acc;
      }, {});
      setSessions(Object.entries(grouped).map(([session_id, v]) => ({ session_id, ...v })));
      setLoading(false);
    };
    load();
  }, []);
