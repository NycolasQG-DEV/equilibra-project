...
export default function QuestionarioPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: userData } = await supabase.from("users").select("admin_id").eq("id", session.user.id).single();
      if (userData?.admin_id) setAdminId(userData.admin_id);

      if (surveyId) {
        const { data: survey } = await supabase.from("surveys").select("title, questions").eq("id", surveyId).single();
        if (survey) {
          setSurveyTitle(survey.title);
          setQuestions(survey.questions as SurveyQuestion[]);
        } else {
  ...
  const handleSubmit = async () => {
    if (!userId || !adminId) { setSubmitted(true); return; }
    setSubmitting(true);
    const riskLabel = risk.level === "Baixo" ? "baixo" : risk.level === "Médio" ? "medio" : "alto";

    await supabase.from("responses").insert([{
      user_id: userId, admin_id: adminId, survey_id: surveyId || null,
      survey_type: "questionario", answers, score: totalScore, risk_level: riskLabel,
    }]);
