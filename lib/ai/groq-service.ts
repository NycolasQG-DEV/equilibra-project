...
import { NR1_DIMENSIONS, calculateRisk, calculateConfidenceScore } from './risk-matrix';
import { SYSTEM_INTERVIEW_PROMPT, buildSynthesisPrompt } from './prompts';
...
function getModelCandidates() {
  const custom = (process.env.GROQ_MODEL || '').replace(/[`'"]/g, '').split('#')[0].split('//')[0].trim();
  const activeOfficialModels = [
    'qwen/qwen3.8-27b',
    'openai/gpt-oss-120b',
    'groq/compound',
    'qwen/qwen3.6-27b',
...
function getModelName() {
  return getModelCandidates()[0] || 'qwen/qwen3.8-27b';
}

function getEconomicalModelName() {
...
export async function getNextInterviewStep(session: any) {
  ...
  // 1. Verifica se a pergunta anterior era um convite prévio para justificar
  ...
  // 2. Detecção de questão de alternativa onde o usuário afirma sofrer/vivenciar risco
  ...
  // 3. Detecção de Resposta Evasiva / Aleatória
  ...
  const messages: any[] = [
    { role: 'system', content: SYSTEM_INTERVIEW_PROMPT },
    {
      ...
      content: JSON.stringify({
        ...
        conversation_history: history.map((h: any, i: number) => ({
          step: i + 1,
          question_asked: h.question,
          worker_answer: h.userAnswer,
          widget_used: h.widgetType,
  ...
  for (const candidateModel of candidateModels) {
    try {
      ...
      console.warn(`⚠️ Modelo Groq (${candidateModel}) falhou:`, err?.message);
      ...
      if (failedGen && typeof failedGen === 'string' && failedGen.trim().length > 0) {
        try {
          const repaired = failedGen.replace(/\\u\{([0-9a-fA-F]+)\}/g, (_: any, hex: string) => {
            try {
...
function buildFallbackStep(step: number, profile: any = {}, context: any = {}) {
  if (context.shouldAskToJustify) {
    return {
      ...
      widget_options: {
        card_left: { label: 'Sim, quero justificar', icon: 'chat' },
        card_right: { label: 'Não, prefiro prosseguir', icon: 'arrow_forward' }
      },
  ...
  switch (step) {
    case 1:
      return {
        ...
        ui_widget: 'binary_cards',
        widget_options: {
          card_left: { label: 'Sim', icon: 'check' },
          card_right: { label: 'Não', icon: 'close' }
        },
        dimension_target: 'demandas_psicologicas'
      };
    case 2:
...
function sanitizeStepResponse(data: any, step: number, profile: any = {}, context: any = {}) {
  ...
  let defaultOptions: any = {
    placeholder: 'Escreva sua opinião com suas próprias palavras...'
  };
  ...
  } else if (widget === 'choice_chips' && (!data.widget_options?.choices || data.widget_options.choices.length === 0)) {
    const qLower = question.toLowerCase();
    if (qLower.includes('família') || qLower.includes('descanso') || qLower.includes('casa') || qLower.includes('rotina')) {
      defaultOptions = {
        choices: ['Não tem impactado', 'Sinto um pouco de cansaço', 'Chego sem energia para a família', 'Impacta muito meu descanso', 'Outro']
      };
    } else if (qLower.includes('lideran') || qLower.includes('chef') || qLower.includes('supervisor')) {
      defaultOptions = {
        choices: ['Suporte respeitoso e aberto', 'Gritos ou tom ríspido', 'Comentários humilhantes', 'Desvalorização de esforço', 'Outro']
      };
    } else if (qLower.includes('assédio') || qLower.includes('respeito')) {
      defaultOptions = {
        choices: ['Nunca presenciei', 'Já presenciei com colegas', 'Já sofri diretamente', 'Prefiro não comentar', 'Outro']
      };
    } else {
      defaultOptions = {
        choices: ['Totalmente adequado', 'Às vezes adequado', 'Pouco adequado', 'Inadequado', 'Outro']
      };
    }
  }
...
export async function generateComprehensiveReport(session: any) {
  const { id, profile = {}, history = [], createdAt = new Date().toISOString() } = session;
  ...
  try {
    ...
    if (!aiSynthesis || Object.keys(aiSynthesis).length === 0) {
      try {
        ...
        if (jsonMatch) {
          const repaired = jsonMatch[0].replace(/\\u\{([0-9a-fA-F]+)\}/g, (_: any, hex: string) => String.fromCodePoint(parseInt(hex, 16)));
          aiSynthesis = JSON.parse(repaired);
        }
      } catch (retryErr: any) {
  ...
  // Consolidação da Matriz de Risco 5x5
  const processedDimensions: any = {};
